...
namespace MobileWebSite.BLL.ToPDF
{
    public class DistributionCreation
    {
        ...
        //private string NameNameOfSender;
        private string NameOfReceive; 
        private int enpriseId;  // = int.Parse(Session["enId"].ToString().Trim());
        public DistributionCreation(Distribution dis, string path, string name, int enpriseid)
        {
            distribution = dis;
            pdfpath = path;
            NameOfReceive = name;
        ...
        public void Createpdf()
        {
            ...
            string NameOfSender = _IEnterpriseRepository.LoadEntities(item => item.Enterprise_ID == enpriseId).FirstOrDefault().Enterprise_Name;
